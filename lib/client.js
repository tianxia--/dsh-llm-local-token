window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function UsageBadge(props) {
			...
			// The badge exists only for these routes: no local token, nothing to show.
			if (state.providers.length === 0) return null;
			const headline = withData
				.flatMap((entry) => entry.usage.windows.map((w) => Math.round(w.used * 100)))
				.slice(0, 2);
			const worst = withData.reduce((max, entry) => Math.max(max, ...entry.usage.windows.map((w) => w.used)), 0);
			...
			return jsxs("span", {
				...
				children: [
					jsxs("button", {
						...
						children: [
							jsx("span", { className: "ltk_dot", style: { background: withData.length === 0 ? "var(--dsw-alias-label-tertiary)" : tone(worst) } }),
					...
					!state.open ? null : jsxs("div", {
						...
						children: [
							...
							...state.providers.map((entry) => jsxs("div", {
								className: "ltk_prov",
								children: [
									jsxs("div", {
		...
		class UsageController {
			constructor() {
				this.state = { open: false, providers: [] };
			...
			async poll() {
				try {
					...
					this.publish({ providers: body.providers ?? [] });
				} catch (_failure) {
			...
			inject() {
				return {
					hooks: { localTokenUsage: this.store },
					start: () => {
		...
		function apply(ctx) {
			...
			// The composer bar is where the context ring lives, so the badge lands
			// next to it without patching the conversation surface.

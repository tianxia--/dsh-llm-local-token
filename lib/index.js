...
export async function apply(ctx, config = {}) {
  ...
  const record = (snapshot) => {
    ...
    diag.observed += 1;
			...
			// Every snapshot is worth keeping, real request or probe alike: the
			// history turns the panel's instant numbers into a trend, and the probe
			// schedule is what lets an unused route participate in it.
			appendHistory(snapshot.provider, snapshot, { days: config.usageHistoryDays });
  };
      ...
      handler: (req, res) => {
        ...
        return sendJson(res, 200, {
          ...
          fetchedAt: new Date().toISOString(),
				...
				wctx.logger.info(`llm-local-token: usage route mounted at ${USAGE_PREFIX}/usage and ${USAGE_PREFIX}/history/<provider>`);

				// History endpoint: the raw stored snapshots for one provider, for the
				// panel's trend view. Reads the same JSONL the probes write.
				wctx.effect(() => wctx.webServer.register({
					kind: "prefix",
					path: USAGE_PREFIX + "/history",
					handler: (req, res) => {
						const path = new URL(req.url ?? "/", "http://localhost").pathname.slice((USAGE_PREFIX + "/history").length).replace(/^\/+/, "");
						const provider = decodeURIComponent(path);
						if (req.method !== "GET" || provider.length === 0 || !/^[a-z0-9-]+$/i.test(provider)) {
							return sendJson(res, 404, { error: "usage: GET " + USAGE_PREFIX + "/history/<provider>" });
						}
						return sendJson(res, 200, { provider, days: config.usageHistoryDays ?? 7, records: readHistory(provider, { days: config.usageHistoryDays ?? 7 }) });
					},
				}), "llm-local-token: history route");
		});
